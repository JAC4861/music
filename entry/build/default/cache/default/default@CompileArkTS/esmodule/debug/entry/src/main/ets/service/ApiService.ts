...
const BASE_URL: string = 'http://192.168.41.26:3000';
// ================== 对外暴露的数据结构 ==================
...
async function getCoverMapBySongDetail(ids: Array<number>): Promise<Map<number, string>> {
    ...
    try {
        ...
        songs.forEach((s: NeteaseSongDetailSong) => {
            const cover: string = toHttps(s.al?.picUrl ?? s.album?.picUrl ?? s.al?.blurPicUrl ?? s.album?.blurPicUrl ?? '');
            if (cover.length > 0) {
                map.set(s.id, cover);
            }
        });
